...
function add_ionization_collisions!(Q, Ie, h_atm, t, n, σ, E_levels, cascading, E, dE, iE, BeamWeight_discrete, μ_center)
    ...
    density_matrix = repeat(n, length(μ_center), length(t))
    ...
    for i_level = 2:size(E_levels, 1)
        if E_levels[i_level, 2] > 0    # these collisions should produce secondary e-
            ...
            if !isempty(i_degrade) && i_degrade[1] < iE
                ...
                Ionizing .= density_matrix .* (σ[i_level, iE] .* @view(Ie[:, :, iE]));
    ...
    for i_level = 2:size(E_levels, 1)
        if E_levels[i_level, 2] > 0    # these collisions should produce secondary e-
            ...
            if !isempty(i_degrade) && i_degrade[1] < iE
                ...
                Ionizing .= density_matrix .* (σ[i_level, iE] .* Ie);
                @views for i_μ in eachindex(μ_center)
                    Ionization[(i_μ - 1) * length(h_atm) .+ (1:length(h_atm)), :] .=
                        max.(0, repeat(n, 1, length(t[iE])) .*
                        (σ[i_level, iE] .*
                        (Ie[(i_μ - 1) * length(h_atm) .+ (1:length(h_atm)), :])) .*
                        BeamWeight_discrete[i_μ] ./ sum(BeamWeight_discrete))
                end
