...
function calculate_cascading_N2(E_grid, dE, lorentzian_width = 11.4)
    ...
    for i_threshold in n_thresholds:-1:1
        ...
        # Define transformation functions for hypercube integration
        E_primary_min(E_degraded) = max(E_primary_range[1], E_degraded + threshold)
        map_to_E_primary(E_degraded, u) = E_primary_min(E_degraded) +
                                           u * (E_primary_range[2] - E_primary_min(E_degraded))
        jacobian_func(E_degraded) = E_primary_range[2] - E_primary_min(E_degraded)

        # Integrand: Lorentzian distribution for secondary electron energy
        function integrand_with_jacobian(vars)
            E_degraded, u_primary = vars
            E_primary_mapped = map_to_E_primary(E_degraded, u_primary)
            E_secondary = E_primary_mapped - threshold - E_degraded

            lorentzian = 1.0 / (E_secondary^2 + lorentzian_width^2)
            jacobian = jacobian_func(E_degraded)
        ...
        for i_primary in i_min_primary:n_energies
            ...
            E_primary_range = [E_primary_bin_min, E_primary_bin_max]
            ...
            if !isnothing(i_max_degraded)
                ...
                for i_degraded in i_max_degraded:(i_primary - 1)
                    ...
                    E_degraded_bin_max_natural = E_grid[i_degraded] + dE[i_degraded]
                    E_degraded_bin_max = min(E_degraded_bin_max_natural,
                                            E_primary_bin_max - threshold)

                    # Integrate only if limits are physical
                    if E_degraded_bin_max > E_degraded_bin_min
                        result, _ = hcubature(integrand_with_jacobian,
...
function calculate_cascading_O2(E_grid, dE, lorentzian_width = 15.2)
    ...
    for i_threshold in n_thresholds:-1:1
        ...
        E_primary_range = Float64[]
        E_primary_min_func(E_degraded) = max(E_primary_range[1], E_degraded + threshold)
        map_to_E_primary(E_degraded, u) = E_primary_min_func(E_degraded) +
                                           u * (E_primary_range[2] - E_primary_min_func(E_degraded))
        jacobian_func(E_degraded) = E_primary_range[2] - E_primary_min_func(E_degraded)
...
function calculate_cascading_O(E_grid, dE)
    ...
    for i_threshold in n_thresholds:-1:1
        ...
        for i_primary in i_min_primary:n_energies
            ...
            # Integrand: empirical distribution for secondary electron energy (atomic oxygen)
            function integrand_with_jacobian(vars)
                E_degraded, u_primary = vars
                E_primary_mapped = map_to_E_primary(E_degraded, u_primary)
                E_secondary = E_primary_mapped - threshold - E_degraded
