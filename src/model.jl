...
"""
    AuroraModel(altitude_lims, θ_lims, E_max, atmosphere, electron, B_angle_to_zenith=0;
                species = (N2Species(atmosphere), O2Species(atmosphere), OSpecies(atmosphere)))
...
- `atmosphere`: source for the default neutral densities. Either a [`NeutralProfile`](@ref)
  (e.g. from [`read_msis_file`](@ref) / [`read_ccmc_msis`](@ref)) or a path to an MSIS text
  file generated by AURORA.
- `electron`: source for the electron background (`ne`, `Te`). Either an [`ElectronProfile`](@ref)
  (e.g. from [`run_iri`](@ref) / [`read_ccmc_iri`](@ref)) or a path to an IRI text file generated
  by AURORA.
...
- `species`: tuple (or any iterable) of [`NeutralSpecies`](@ref) instances that the model
  should simulate. Defaults to the standard atmosphere: N₂, O₂, O from `atmosphere`.
  Pass a custom tuple to add, remove, or replace species:
  ```julia
  atm = read_msis_file(msis_file)

  # Two species example:
  model = AuroraModel(...; species = (O2Species(atm), OSpecies(atm)))
  ...
  model = AuroraModel(...; species = (N2Species(atm), O2Species(atm),
...
"""
function AuroraModel(altitude_lims, θ_lims, E_max, atmosphere, electron, B_angle_to_zenith=0;
                     species = (N2Species(atmosphere), O2Species(atmosphere),
                                OSpecies(atmosphere)))
