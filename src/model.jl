...
- `neutrals`: source for the default neutral densities. Either a [`NeutralAtmosphere`](@ref)
  (e.g. from [`run_msis`](@ref) / [`read_msis_file`](@ref) / [`read_ccmc_msis`](@ref)) or a
  path to an MSIS text file generated by AURORA.
...
- `species`: tuple (or any iterable) of [`NeutralSpecies`](@ref) instances that the model
  should simulate. Defaults to the standard atmosphere: N₂, O₂, O from `neutrals`.
