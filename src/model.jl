...
"""
    AuroraModel(altitude_lims, θ_lims, E_max, neutrals, electrons, B_angle_to_zenith=0;
...
- `neutrals`: source for the default neutral densities, from which the standard N₂/O₂/O
  species are built. Either a [`NeutralAtmosphere`](@ref) (e.g. from [`run_msis`](@ref) /
  [`read_msis_file`](@ref) / [`read_ccmc_msis`](@ref)) or a path to an MSIS text file
  generated by AURORA. Pass `nothing` when `species` is given explicitly, since each
  species then carries its own density source.
- `electrons`: source for the electron background (`ne`, `Te`). Either an
  [`ElectronProfile`](@ref) (e.g. from [`run_iri`](@ref) / [`read_iri_file`](@ref) /
  [`read_ccmc_iri`](@ref)) or a path to an IRI text file generated by AURORA.
...
- `species`: tuple (or any iterable) of [`NeutralSpecies`](@ref) instances that the model
  should simulate. When omitted, the model is built from `neutrals`: the standard
  atmosphere, N₂, O₂, O. Pass a custom tuple to add, remove, or replace species, and pass
  `neutrals = nothing` since it is not used in that case:
  ```julia
  atm = read_msis_file(msis_file)
...
function AuroraModel(altitude_lims, θ_lims, E_max, neutrals, electrons, B_angle_to_zenith=0;
                     species = nothing)
    if species === nothing && neutrals === nothing
        throw(ArgumentError("AuroraModel needs either a neutral atmosphere (`neutrals`) " *
                             "or an explicit `species` tuple"))
    elseif species !== nothing && neutrals !== nothing
        throw(ArgumentError("`neutrals` is not used when `species` is given; pass " *
                             "`neutrals = nothing` (the species already carry their own " *
                             "density sources)"))
    end
    if species === nothing
    ...
    ionosphere       = Ionosphere(electrons, altitude_grid.h)
